    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        ...
        if (msg.conversation_id === selectedConvId) {
          // React Query handles invalidation via onSuccess, 
          // but for instant feedback from external senders:
          refetchConvs();
        } else {
          refetchConvs();
        }
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        <span className="text-14 font-medium text-foreground truncate">{conv.otherUser.full_name}</span>
                        {conv.otherUser.kyc_level >= 2 && <BadgeCheck className="w-3.5 h-3.5 text-primary flex-shrink-0" />}
                      </div>
        ...
        <div className={`${selectedConvId ? 'flex' : 'hidden md:flex'} flex-col flex-1 bg-background`}>
          {selectedConvId && activeConv ? (
            <>
              ...
              <div className="bg-surface border-b border-border px-4 py-3 flex items-center gap-3 shadow-xs">
                ...
                <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center text-primary font-heading font-bold text-14 overflow-hidden">
                  {activeConv.otherUser.avatar_url ? (
                    ...
                    activeConv.otherUser.full_name?.[0]?.toUpperCase() || 'U'
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
              ...
              {activeConv.listing && (
                <Link to={`/annonce/${activeConv.listing.id}`} className="bg-surface border-b border-border px-4 py-2 flex items-center gap-3 hover:bg-background transition-colors">
